package com.example.navipilot.utils
...
object CoordinatePreferences {
     ...
     * 保存停车位置
     */
    fun saveParkedLocation(prefs: SharedPreferences, latitude: Double, longitude: Double) {
        prefs.edit().apply {
            putString(KEY_LAST_PARKED_LAT, latitude.toString())
            putString(KEY_LAST_PARKED_LON, longitude.toString())
            putLong(KEY_LAST_PARKED_TIME, System.currentTimeMillis())
            apply()
        }
    }
     ...
     * 读取停车位置
     */
    fun getParkedLocation(prefs: SharedPreferences): ParkedLocation? {
        val latStr = prefs.getString(KEY_LAST_PARKED_LAT, null) ?: return null
        val lonStr = prefs.getString(KEY_LAST_PARKED_LON, null) ?: return null
        val timestamp = prefs.getLong(KEY_LAST_PARKED_TIME, 0)

        val lat = latStr.toDoubleOrNull() ?: return null
        val lon = lonStr.toDoubleOrNull() ?: return null

        return if (lat != 0.0 && lon != 0.0 && timestamp > 0) {
            ParkedLocation(lat, lon, timestamp)
        } else {
            null
        }
    }
     ...
     * 清除停车位置
     */
    fun clearParkedLocation(prefs: SharedPreferences) {
        prefs.edit().apply {
            remove(KEY_LAST_PARKED_LAT)
            remove(KEY_LAST_PARKED_LON)
            remove(KEY_LAST_PARKED_TIME)
            apply()
        }
    }

    private const val KEY_LAST_PARKED_LAT = "last_parked_lat"
    private const val KEY_LAST_PARKED_LON = "last_parked_lon"
    private const val KEY_LAST_PARKED_TIME = "last_parked_time"
}
